...
// Render the measured pitch analysis (from the learner's actual voice) into a prompt
// block so Lin Wei can coach the specific mis-toned syllables.
function pronunciationBlock(p) {
  if (!p || !Array.isArray(p.syllables) || !p.syllables.length) return "";
  const rows = p.syllables
    .map((s) => {
      if (s.match === null) return `${s.char} (${s.pinyin}): neutral, not scored`;
      const mark = s.match ? "OK" : "WRONG";
      return `${s.char} (${s.pinyin}): produced tone ${s.detected || "?"}, should be tone ${s.expected} [${mark}]`;
    })
    .join("\n");
  return `
The learner SPOKE this reply. A pitch analysis of their actual voice measured these tones per syllable:
${rows}
Measured tone accuracy: ${p.matches ?? 0}/${p.total ?? 0} syllables correct.
In your reply, briefly and kindly coach the specific WRONG syllables (name the character and which way the pitch should go). Do not invent tone errors beyond the measured ones.`;
}

export async function chatTurn(scenario, level, history, userMessage, openingZh = "", pronunciation = null) {
  const lines = [];
...
Conversation so far:
${historyText}
Learner's latest message: "${userMessage}"${pronunciationBlock(pronunciation)}
