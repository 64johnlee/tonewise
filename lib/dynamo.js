...
const TABLE = process.env.DYNAMODB_TABLE || "tonetutor";
let _doc;
function doc() {
  if (!_doc) {
...
export async function getStatus(uid) {
  ...
  const u = r.Item || {};
  return { paid: !!u.paid, free_used: u.free_used || 0, free_limit: FREE_LIMIT, streak: u.streak || 0 };
}
export async function consumeIfAllowed(uid) {
  ...
  await doc().send(new UpdateCommand({
    TableName: TABLE, Key: { PK: `USER#${uid}`, SK: "PROFILE" },
